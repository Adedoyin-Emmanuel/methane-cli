...
const generateServiceWorkerFile = async () => {
    if (SWDir) {
        try {
            ...
            await fs.writeFile(path.resolve(SWDir, `service-worker.js`), swResolver.resolveServiceWorkerTemplate(), (error) => {
                if (error) {
                    console.log(colors.bold(colors.red(error.toString())));
                }
            });
            await fs.writeFile(path.resolve(SWDir, "serviceWorkerRegistration.js"), swResolver.resolveServiceWorkerRegistrationTemplate(), (error) => {
                if (error) {
                    console.log(colors.bold(colors.red(error.toString())));
                }
            });
    ...
    console.log(`${colors.bold(colors.green(`Service Worker generated successfully`))}`);
