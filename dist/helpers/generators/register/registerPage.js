...
export const addPageImport = (rootDir, pageName, currentPageName) => {
    ...
    const generatedPageImportStatement = readUserConfig.readConfig().generateFolder === "true"
        ...
        if (supportedExtensions.includes(fileExt)) {
            ...
            if (!fileContents.includes(generatedPageImportStatement)) {
                ...
                if (importStatements && importStatements.length > 0) {
                    ...
                    const lastImportStatementIndex = newContents.lastIndexOf(lastImportStatement);
                    newContents =
                        newContents.slice(0, lastImportStatementIndex + lastImportStatement.length) +
                            "\n" +
                            generatedPageImportStatement +
                            newContents.slice(lastImportStatementIndex + lastImportStatement.length);
                }
                else {
                    newContents = generatedPageImportStatement + "\n" + newContents;
                }
                fs.writeFileSync(filePath, newContents);
                console.log(colors.cyan(colors.bold(`${pageName} Page Successfully Added to ${filePath}`)));
            }
            else {
                console.log(colors.bold(colors.red(`Import Statement Already in ${filePath}`)));
            }
        }
    };
