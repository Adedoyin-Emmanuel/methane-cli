...
const generatePageFile = async (name, pageDir, pageResolver) => {
    await fs.writeFile(pageDir, pageResolver.resolvePageContent(readUserConfig, name), (error) => {
        if (error) {
...
export const generatePage = async (name) => {
    ...
    const pageDirName = readUserConfig.readConfig().generateFolder === "true"
        ...
        : path.join(pageDir);
    const pageFilePath = path.join(pageDirName + `/${name}.${readUserConfig.readConfig().template}`);
    try {
        ...
        if (readUserConfig.readConfig().generateStylesheet === "true") {
            ...
            const cssFilePath = path.join(pageDirName, `${name}Style.${styleSheetType}`);
        ...
        readUserConfig.readConfig().register === "true"
            ? pageRegisterer.addPageImport(rootDir, name, path.join(".", pageDir.replace("src/", "")))
            : null;
    }
    catch (error) {
